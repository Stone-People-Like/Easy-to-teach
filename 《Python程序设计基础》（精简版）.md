...
- 程序设计语言
  ...
  -  低级语言（机器语言，汇编语言）
    ...
    机器语言：是由计算机处理器执行的最基础的语言，它完全有二进制代码（1,0）组成，并且与硬件架构密切相关

    汇编语言：又称符号语言，是一种符号化的机器语言，使用计算机助记符（如`MOV`，`ADD`）来代替二进制代码，需要通过**汇编器**转换成机器语言。常用语需要精确控制硬件操作的程序，如嵌入式系统，驱动程序开发等
  ...
  - 高级语言
    ...
    | `Java`       | 跨平台，面对对象，强大的生态系统       | 企业级应用，安卓系统开发              |
...
2. Python特点
   ...
   - 解释性语言
     ...
     解释型语言指的是程序在执行时由**解释器逐行将源代码转换为机器码**，然后自立即执行，**无须事先编译**。
...
- `turtle` 模块的常用语句
  ...
  - turtle模块常见动作（`turtle`．后所跟的＜语句＞）
    ...
    | `color（参数 m，参数 n）` | 表示设置颜色，**m表示画笔颜色（若省略于n相同），n表示填充颜色** |
    | `begin＿fill（）与end＿fill（）` | 表示在**封闭图形**中开始填充颜色与结束填充颜色，**需成对出** |
